...
class Grappa(object):
    ...
    def get_kernel_geometries(self):
        ...
        with NTF() as fP:
            ...
            P = np.memmap(
                fP, dtype=mask.dtype, mode="w+", shape=(mask.shape[0] - 2 * kx2, mask.shape[1] - 2 * ky2, 1, kx, ky)
            )
    ...
    def compute_weights(self, calib: np.ndarray) -> dict[int, np.ndarray]:
        ...
        with NTF() as fA:
            ...
            weights = self.compute_weights_from_patches(A, kx2, ky2)

        return weights

    def compute_weights_from_patches(self, A: np.ndarray, kx2: int, ky2: int) -> dict[int, np.ndarray]:
        weights = {}
...
def grappa_reconstruction(
    kspace_data: np.ndarray, calib_data: np.ndarray, kernel_geometry_slice: int = 0
) -> np.ndarray:
    """Perform GRAPPA technique on k-space data.

    Parameters
    ----------
    kspace_data : numpy.ndarray
        Input k-space data with shape (num_slices, num_coils, num_rows, num_cols).
    calib_data : numpy.ndarray
        Calibration data for GRAPPA with shape (num_slices, num_coils, num_rows_cal, num_cols_cal).

    Returns
    -------
    ksapce_post_grappa : numpy.ndarray
        k-space with shape (num_slices, num_coils, num_rows, num_cols).

    """

    kspace_data = kspace_data.transpose(0, 3, 1, 2)
    calib_data = calib_data.transpose(0, 3, 1, 2)

    grappa_obj = Grappa(kspace_data[kernel_geometry_slice], kernel_size=(5, 5), coil_axis=1)

    kspace_post_grappa = np.zeros(shape=kspace_data.shape, dtype=complex)
    for slice_num in range(kspace_data.shape[0]):
        ...
        kspace_post_grappa[slice_num] = grappa_obj.apply_weights(kspace_data[slice_num], grappa_weight_dict)

    return kspace_post_grappa.transpose(0, 2, 3, 1)
